...
for split, in_loader in [('train', trainloaderIn), ('val', testloaderIn),]:
    ...
    if FORCE_RUN or not os.path.exists(cache_name):
        ...
        logit_log = np.zeros((len(in_loader.dataset), num_classes))
        ...
        for batch_idx, (inputs, targets) in enumerate(in_loader):
            ...
            if Classifier_RUN:
                lslayer_feat=model.encoder(inputs)
                score = classifier(lslayer_feat)
            ...
            logit_log[start_ind:end_ind] = score.data.cpu().numpy()
            if batch_idx % 100 == 0:
                print(f"{batch_idx}/{len(in_loader)}")
        np.savez(cache_name, feat_log=feat_log, logit_log=logit_log, label_log=label_log)
    else:
        log = np.load(cache_name, allow_pickle=True)
        feat_log, logit_log, label_log = log["feat_log"], log["logit_log"], log["label_log"]
...
for ood_dataset in args.out_datasets:
    ...
    if FORCE_RUN or not os.path.exists(cache_name):
        ...
        ood_logit_log = np.zeros((len(out_loader.dataset), num_classes))
        ...
        for batch_idx, (inputs, _) in enumerate(out_loader):
            ...
            ood_logit_log[start_ind:end_ind] = score.data.cpu().numpy()
            if batch_idx % 100 == 0:
                print(f"{batch_idx}/{len(out_loader)}")
        np.savez(cache_name, ood_feat_log=ood_feat_log, ood_logit_log=ood_logit_log)
    else:
        log = np.load(cache_name, allow_pickle=True)
        ood_feat_log, ood_logit_log = log["ood_feat_log"], log["ood_logit_log"]
