...
def get_classifier(args, num_classes, load_ckpt=True, load_epoch=None):
    ...
    if args.model_arch == 'resnet50-supcon':
        ...
        ckpt = torch.load("checkpoints/classifier/imagenet/11.pth")
        classifier.load_state_dict(ckpt["model"])
    elif args.model_arch == 'resnet18-supcon':
        classifier = LinearClassifier(name="resnet18", num_classes=num_classes)
        ckpt = torch.load("checkpoints/classifier/cifar-10/cifar_classifer.pth")
        classifier.load_state_dict(ckpt["model"])
    classifier.cuda()
    return classifier
